 ...
 * 
 * <li>One or more {@link LeafFactory} objects are needed. They are called
 * by the parser whenever a token code provided by the lexer is recognized.
 * The parser will call {@link LeafFactory#create} with the lexer as
 * parameter to obtain an {@code N} for the token (code). Which factory is
 ...
 * create an choice of subnodes. A rough outline of an example is:
 * </p>
 * 
 * <pre>
 * GrammarBuilder&lt;...&gt; gb = 
 ...
 * Parser&lt;...&gt; parser = product.compile();</pre>
 * 
 * <p>
 * This would define a {@code product} as arbitrary length sequence of
 * {@code NUMBER}s. When the parser recognizes {@code CODE_NUMBER}, it
 * calls the {@code leafFactory}. Later, when it has collected all
 * {@code NUMBER}s available, it will call the {@code nodeFactory} with the
 * list of results provided by the calls to {@code leafFactory}. The result
 * of the parse would be whatever the {@code nodeFactory} makes out of the
 * list. It could return an object that contains the list as a field, but it
 * could as well multiply the numbers and return an {@code N} that contains
 * just the product.
 * </p>
 * <p>
 ...
 * {@link absimpa.Lexer}. This is up to you.
 * </p>
 * 
...
public class GrammarBuilder<N,C extends Enum<C>,L extends Lexer<C>> {
   ...
   */
  public GrammarBuilder(NodeFactory<N> defaultFactory, LeafFactory<N,C,L> defaultLeaf) {
   ...
   * token into the result type {@code N}.</p>
   */
   ...
   * {@code g}. The provided {@code factory} will be used to transform
   * the recognized list of elements into the result type {@code N}. 
   * 
  ...
  /**
   * convenience function to call {@link #repeat(Grammar,int,int)} with {@code min=0}
   * and {@code max=Integer.MAX_VALUE}.
   */
  public Repeat<N,C,L> star(Grammar<N,C,L> grammar) {
    return repeat(grammar, 0, Integer.MAX_VALUE);
  }
  /**
   * convenience function to call {@link #repeat(NodeFactory,Grammar,int,int)} with {@code min=0}
   * and {@code max=Integer.MAX_VALUE}.
   */
   ...
   * call {@link Choice#or}. In contrast to all other grammars, the
   * {@code Choice} does not need a {@link NodeFactory}, because the
   * resulting parser just passes on the choice that was recognized.
   */
